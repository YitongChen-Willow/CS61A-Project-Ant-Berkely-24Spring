...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        i_place = self.place
        i = 0
        while not i_place.is_hive:
            if i < self.lower_bound:
                i, i_place = i+1, i_place.entrance
                continue
            if i > self.upper_bound:
                break
            if i_place.bees != []:
                near_bee = random_bee(i_place.bees)
                return near_bee
            i, i_place = i+1, i_place.entrance
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        updated_damage = amount
        if self.place is None:
            updated_damage += self.damage
        bees = list(place.bees)
        for bee in bees:
            bee.reduce_health(updated_damage)
...
class HungryAnt(Ant):
    """HungryAnt chews or eats a random bee in its place."""
...
class TankAnt(ContainerAnt):
    """TankAnt provides protection and does damage."""
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        i_place = self.place
        if i_place.ant is not self:
            i_place.ant.damge = i_place.ant.damage * 2
        i_place = i_place.exit
        while i_place is not None:
            if i_place.ant:
                ant = i_place.ant 
                if ant.is_doubled is False:
                    ant.damage = ant.damage * 2
                    ant.is_doubled = True
                if (ant.is_container == True) and (ant.ant_contained) \
                    and (ant.ant_contained.is_doubled is False):
                    ant.ant_contained.damage = ant.ant_contained.damage * 2
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        i_distance = 0
        if i_place.ant is not self:
            insect[i_place.ant] = i_distance
        for bee in i_place.bees:
            insect[bee] = i_distance
        i_distance, i_place = i_distance + 1, i_place.entrance
        while i_place.is_hive is False:
            if i_place.ant:
                insect[i_place.ant] = i_distance
                if (i_place.ant.is_container) and \
                    (i_place.ant.ant_contained is not None):
                    insect[i_place.ant.ant_contained] = i_distance
            for bee in i_place.bees:
                insect[bee] = i_distance
            i_distance, i_place = i_distance + 1, i_place.entrance
        return insect
    ...
    def calculate_damage(self, distance):
        damage = self.damage - self.insects_shot * self.derate \
            - distance * self.derate_dis
        if damage <= 0:
            return 0
        return damage
